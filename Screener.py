...
def scraper(token):
  ...
  for row in soc:
      ...
      if 'ROCE' in row_text or 'Return on Capital Employed' in row_text or 'Return on capital employed' in row_text:
          roce_row = row
          break

  ROCE = []
  if roce_row:
      for s in roce_row.find_all('td', class_=''):
          txt = s.text.strip()
          if not txt:
              continue
          try:
              ROCE.append(int(txt.replace('%', '').replace(',', '')))
          except ValueError:
              continue
